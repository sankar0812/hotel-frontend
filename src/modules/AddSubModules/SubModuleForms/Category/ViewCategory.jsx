...
import { deleteCategory, getCategory } from '@modules/AddSubModules/SubModulesSlice'
...
const ViewCategory = ({ active }) => {
  ...
  const { category, status, error } = useSelector((state) => state.subModule)
  ...
  const Updatecategory = (record) => {
    ...
    setModalContent(<AddCategory allcategory={allcategory} trigger={trigger} formname={'updateCategory'} record={record} handleOk={handleOk} />)
    showModal();
  }
