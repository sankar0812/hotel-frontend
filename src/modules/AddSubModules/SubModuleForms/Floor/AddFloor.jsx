...
const AddFloor = ({ allFloors, formname, record, handleOk, trigger }) => {
  ...
  const add = (values) => {
    baseRequest.post(APIURLS.ADDFLOOR, values)
      .then(resp => {
  ...
  const update = (values) => {
    baseRequest.put(`${APIURLS.EDITFLOOR}${record.floorId}`, values)
      .then(resp => {
        console.log(resp.data, 'responnse');
        form.resetFields();
        handleOk()
  ...
  return (
    <Form
      ...
      <CustomRow space={[24, 24]}>
        ...
        <Col span={24} md={24}>
          <CustomInput label={'Floor Name'} placeholder={'Enter floor Name'} name={'floorName'}
            rules={[
              {
                required: true,
