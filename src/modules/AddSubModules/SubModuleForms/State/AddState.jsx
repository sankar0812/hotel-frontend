...
const AddState = ({ allStates, formname, record, handleOk, trigger }) => {
    ...
    const add = (values) => {
        baseRequest.post(APIURLS.ADDSTATE, values)
            .then(resp => {
    ...
    const update = (values) => {
        baseRequest.put(`${APIURLS.EDITSTATE}${record.stateId}`, values)
            .then(resp => {
                console.log(resp.data, 'responnse');
                form.resetFields();
                handleOk()
    ...
    return (
        <Form
            ...
            <CustomRow space={[24, 24]}>
                ...
                <Col span={24} md={24}>
                    <CustomInput label={'State Name'} placeholder={'Enter Floor Name'} name={'stateName'}
                        rules={[
                            {
                                required: true,
